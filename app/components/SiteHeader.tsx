...
export default function SiteHeader() {
  ...
  return <header className="nav-shell">
    <Link className="brand" href="/" aria-label="返回事为电商首页"><img src="/assets/brand/logo-white.png" alt="事为电商 SellsWell E-commerce" /></Link>
    <nav ref={navRef} className={menuOpen ? "mobile-open" : ""} aria-label="主导航">
      {nav.map(([href, label]) => <Link className={pathname === href ? "active" : ""} href={href} key={href} onClick={() => setMenuOpen(false)}>{label}</Link>)}
