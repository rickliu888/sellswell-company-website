...
export default function Home() {
  ...
  return <main>
    ...
    <section className="team-section" id="about"><div className="team-images"><img className="team-main" src="/assets/team/team-cliff-hd-16x9.jpg" alt="事为与八千里路团队海边山崖团建"/><img className="team-small" src="/assets/team/team-coast.jpg" alt="事为与八千里路团队团建"/></div><div className="team-copy"><p className="eyebrow dark"><span/> PEOPLE & CULTURE</p><h2>{lang==="zh"?"年轻、专业，一起把事情做成":"Young, professional and built to deliver"}</h2><p>{lang==="zh"?"我们来自互联网与跨境一线，相信长期主义，也愿意让优秀伙伴共享公司的长期成长。":"We come from frontline internet and global commerce teams, believe in long-term value, and share growth with exceptional people."}</p><a href="/about">{lang==="zh"?"走进事为团队":"Meet our team"} ↗</a></div></section>
    ...
    {contactOpen && <div className="modal-backdrop" role="presentation" onMouseDown={()=>setContactOpen(false)}><div className="contact-modal" role="dialog" aria-modal="true" aria-label="联系我们" onMouseDown={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setContactOpen(false)} aria-label="关闭">×</button><p className="eyebrow dark"><span/> BUSINESS CONTACT</p><h2>{lang==="zh"?"微信咨询":"WeChat consultation"}</h2><img src="/assets/contact/wechat-business.png" alt="商务合作微信二维码"/><strong>{lang==="zh"?"请注明公司名称以及来意，谢谢":"Please include your company name and purpose."}</strong><div className="copy-row"><code>RickLiu1992</code><button onClick={()=>copyValue("RickLiu1992","微信号")}>{copied?`${copied}已复制`:lang==="zh"?"复制微信号":"Copy ID"}</button></div><a className="modal-email" href="mailto:rick@sellswell.cn">rick@sellswell.cn ↗</a></div></div>}
  </main>;
}
